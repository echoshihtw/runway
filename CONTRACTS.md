...
| 2026-09 | `ITSAppUsesNonExemptEncryption` is `true`, not `false` | The key was `false`, which tells App Store Connect there is nothing to ask about and skips the compliance questions entirely — so the 2026-06 row above could never be carried out. Apple's rule is that `false` is only correct when the app uses no encryption, or only exempt forms: platform cryptography, HTTPS, authentication, or copy protection. SQLCipher is none of those. It is a third-party AES library compiled into the binary, and Zetetic classify it as mass market under ECCN 5D992.c. So the app does use encryption Apple counts, it qualifies for License Exception ENC under EAR 740.17(b)(1) as mass market, and the way to say both is `true` plus the exemption answers. Two obligations follow, both on the owner: answer the export compliance questions once in App Store Connect (standard algorithms, contains third-party cryptography, mass market), and file the annual self-classification report to BIS and the ENC Encryption Request Coordinator listing this app |
